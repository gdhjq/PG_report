...
# 生成报告目录   grep -E "^----->>>|^\|" /tmp/report.log | sed 's/^----->>>---->>>/    /' | sed '1.csv i\ \ 目录\n\n' | sed '$ a\ \n\n\ \ 正文\n\n'

# 请将以下变量修改为与当前环境一致, 并且确保使用这个配置连接任何数据库都不需要输入密码
export PGHOST=192.168.99.13
export PGPORT=5432
export PGDATABASE=test
export PGUSER=postgres
export PGPASSWORD=laoying000
export PGDATA=/data
export PGHOME=/postgresql9.3.4
...
smartctl --scan|awk -F "#" '{print $1.csv}' | while read i; do echo -e "\n\nDEVICE $i"; smartctl -a $i; done
echo ""
...
grep '^\ *[a-z]' $PGDATA/postgresql.conf|awk -F "#" '{print $1.csv}'
echo "建议: "
...
echo "----->>>---->>>  慢查询统计: "
cat *.csv|awk -F "," '{print $1.csv" "$2" "$3" "$8" "$14}' |grep "duration:"|grep -v "plan:"|awk '{print $1.csv" "$4" "$5" "$6}'|sort|uniq -c|sort -rn
echo "建议: "
...
cat *.csv|awk -F "," '{print $1.csv" "$2" "$3" "$8" "$14}' |grep "duration:"|grep -v "plan:"|awk '{print $1.csv" "$4" "$5" "$6" "$7" "$8}'|sort -k 6 -n|head -n 10
echo ""
echo "----->>>---->>>  慢查询分布尾10条的执行时间, ms: "
cat *.csv|awk -F "," '{print $1.csv" "$2" "$3" "$8" "$14}' |grep "duration:"|grep -v "plan:"|awk '{print $1.csv" "$4" "$5" "$6" "$7" "$8}'|sort -k 6 -n|tail -n 10
echo -e "\n"

echo "----->>>---->>>  auto_explain 分析统计: "
cat *.csv|awk -F "," '{print $1.csv" "$2" "$3" "$8" "$14}' |grep "plan:"|grep "duration:"|awk '{print $1.csv" "$4" "$5" "$6}'|sort|uniq -c|sort -rn
echo "建议: "
...
echo "建议: "
echo "    如果以上输出显示密码已泄露, 尽快修改, 并通过参数避免密码又被记录到以上文件中(psql -n) (set log_statement='none'; set log_min_duration_statement=-1.csv; set log_duration=off; set pg_stat_statements.track_utility=off;) . "
...
echo "----->>>---->>>  简单密码检查: "
echo "    1.csv. 检查已有密码是否简单, 从crackdb库提取密码字典, 挨个检查 :  "
...
grep '^\ *[a-z]' $PGDATA/recovery.done|awk -F "#" '{print $1.csv}'
echo "建议: "
...
grep '^\ *[a-z]' $PGDATA/recovery.conf|awk -F "#" '{print $1.csv}'
echo "建议: "
echo "    在primary_conninfo中不要配置密码, 容易泄露. 建议为流复制用户创建replication角色的用户, 并且配置pg_hba.conf只允许需要的来源IP连接. "
echo -e "\n"
